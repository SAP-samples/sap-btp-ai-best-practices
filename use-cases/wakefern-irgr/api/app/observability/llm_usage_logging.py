...
class TokenUsage:
    ...
    Attributes:
        ...
        cached_input_tokens: Input tokens served from a provider prompt cache.
...
def _cached_input_tokens(usage: Any) -> int:
    """Return cached input tokens from native, Responses API, or LangChain usage."""
    cached = _first_numeric_value(
        usage,
        (
            "cached_input_tokens",
    ...
    for details_key in (
        ...
        details = _read_value(usage, details_key)
        cached = _first_numeric_value(
            details,
            ("cached_tokens", "cache_read", "cacheRead"),
        )
        if cached:
            return cached
...
def usage_dict_from_tokens(usage: TokenUsage) -> dict[str, int]:
    ...
    return {
        ...
        "cached_prompt_tokens": int(usage.cached_input_tokens or 0),
...
def emit_llm_usage_event(
    *,
    ...
    Args:
        ...
        cached_input_tokens: Input token count served from provider cache.
