...
class PageRouter {
  ...
  async loadPage(pageName, ctx = null) {
    // Skip reload check if ctx has force flag, or if this is spa-quick-lookup (always reload for navigation)
    const shouldSkipReload = this.currentPage === pageName && !ctx && pageName !== 'spa-quick-lookup';

    if (shouldSkipReload) {
      console.log(`[Router] Already on ${pageName}, skipping reload`);
      return;
    }
