...
from gen_ai_hub.proxy.langchain.google_genai import ChatGoogleGenerativeAI  # type: ignore


def make_llm(
    provider: Literal["openai", "bedrock", "google"],
    model_name: str,
    ...
    - provider: which backend to use ("openai", "bedrock", "google")
    - model_name: proxy model name, e.g. "gpt-4o", "anthropic--claude-3.5-sonnet", "gemini-2.5-flash", "sonar-pro"
    ...
    if provider == "google":
        # Use Google Generative AI (Gemini) via GenAI Hub
