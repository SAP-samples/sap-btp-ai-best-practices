...
def prepare_order_data(row: pd.Series, features_df: pd.DataFrame = None) -> Dict[str, Any]:
    ...
    order_data = {
        ...
        "model_anomaly_score": float(row.get('anomaly_score', 0)),
        "model_predicted_anomaly": int(row.get('predicted_anomaly', 0)),
        
        # Key Order Features (Actual Values)
        "actual_sales_order_item_qty": float(row.get('Sales Order item qty', 0)),
        "actual_unit_price": float(row.get('Unit Price', 0)),
        "actual_order_item_value": float(row.get('Order item value', 0)),
        "actual_current_month_total_qty": float(row.get('current_month_total_qty', 0)),
        "actual_month_rolling_z": float(row.get('month_rolling_z', 0)) if 'month_rolling_z' in row else None,
        "actual_order_share_of_month": float(row.get('order_share_of_month', 0)) if 'order_share_of_month' in row else None,
        "actual_fulfillment_duration_days": float(row.get('fulfillment_duration_days', 0)),
        "actual_qty_z_score": float(row.get('qty_z_score', 0)),
        "actual_qty_deviation_from_mean": float(row.get('qty_deviation_from_mean', 0)),
        
        # Comparative Benchmarks & Historical Context
        "typical_qty_p05": float(row.get('p05', 0)),
        "typical_qty_p95": float(row.get('p95', 0)),
        "historical_median_qty_for_material": float(row.get('hist_median', 0)) if 'hist_median' in row else None,
        "num_historical_orders_for_material": int(row.get('num_historical_orders', 0)) if 'num_historical_orders' in row else None,
        "typical_price_p05": float(row.get('price_p05', 0)),
        "typical_price_p95": float(row.get('price_p95', 0)),
        "expected_order_item_value_model": float(row.get('expected_order_item_value', 0)),
        "month_rolling_mean": float(row.get('month_rolling_mean', 0)) if 'month_rolling_mean' in row else None,
        "month_rolling_std": float(row.get('month_rolling_std', 0)) if 'month_rolling_std' in row else None,
        "typical_fulfillment_p05": float(row.get('fulfillment_p05', 0)),
        "typical_fulfillment_p95": float(row.get('fulfillment_p95', 0)),
        "ship_to_percentage_for_sold_to": float(row.get('ship_to_percentage_for_sold_to', 0)) if 'ship_to_percentage_for_sold_to' in row else None,
        "typical_qty_z_score_range": "-2 to +2",
        
        # Boolean Anomaly Flags
        "flag_is_first_time_cust_material_order": bool(row.get('is_first_time_cust_material_order', False)),
        "flag_is_rare_material": bool(row.get('is_rare_material', False)),
        "flag_is_suspected_duplicate_order": bool(row.get('is_suspected_duplicate_order', False)),
        "flag_is_unusual_unit_price": bool(row.get('is_unusual_unit_price', False)),
        "flag_is_qty_outside_typical_range": bool(row.get('is_qty_outside_typical_range', False)),
        "flag_is_unusual_fulfillment_time": bool(row.get('is_unusual_fulfillment_time', False)),
        "flag_is_order_qty_high_z": bool(row.get('is_order_qty_high_z', False)) if 'is_order_qty_high_z' in row else False,
        "flag_is_value_mismatch_price_qty": bool(row.get('is_value_mismatch_price_qty', False)),
        "flag_is_unusual_uom": bool(row.get('is_unusual_uom', False)),
        "flag_is_unusual_ship_to_for_sold_to": bool(row.get('is_unusual_ship_to_for_sold_to', False)) if 'is_unusual_ship_to_for_sold_to' in row else False,
    ...
    if features_df is not None and 'Material Number' in features_df.columns:
        ...
        if not material_data.empty and 'Sales Order item qty' in material_data.columns:
            order_data["historical_median_qty_for_material"] = float(material_data['Sales Order item qty'].median())
