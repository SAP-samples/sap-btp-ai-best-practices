...
def load_and_preprocess_data(data_directory='data'):
    ...
    """
    # Get list of CSV files ending with '.csv' and sort them by filename
...
def add_monthly_volume_features(df):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        df['order_share_of_month'] = df['Sales Order item qty'] / df['current_month_total_qty']
    df['order_share_of_month'] = df['order_share_of_month'].fillna(0)
...
def add_pricing_features(df):
    ...
    mad_pos = enough_cust & (df['cust_price_mad'] > 0)
