...
EIP_BaseLogisticsCostRate_Param = 0.15 # Example: 15% of unit price
...
def calculate_price_trend(group, mapped_price_col, mapped_date_col):
    ...
    time_span_days = time_delta.max() - time_delta.min()

    # Enhanced validation to prevent unrealistic extrapolations
    # Require at least 3 unique dates and minimum 30-day span for reliable trends
    if time_delta.nunique() < 3:
        return 0.0 # Need at least 3 different dates
    
    if time_span_days < 30:
        return 0.0 # Need at least 30 days of data for reliable trend
    ...
    try:
        ...
        # Additional validation: reject trends with poor statistical significance
        if pd.isna(slope) or pd.isna(r_value) or abs(r_value) < 0.3:
            return 0.0  # Weak correlation, trend not reliable
        
        # Cap extreme slopes to prevent unrealistic extrapolations
        # Max reasonable price change: 50% per year = 0.137% per day
        avg_price = series_clean.mean()
        max_reasonable_slope = avg_price * 0.5 / 365  # 50% annual change
        min_reasonable_slope = -max_reasonable_slope
        
        # Cap the slope to reasonable bounds
        capped_slope = max(min_reasonable_slope, min(max_reasonable_slope, slope))
        
        # Log if we're capping an extreme value
        if abs(slope) > abs(max_reasonable_slope):
            print(f"Warning: Capping extreme price trend slope from {slope:.4f} to {capped_slope:.4f} for reliability")
...
def calculate_logistics_cost(row, country_col, material_col, logistics_data, base_rate):
    """Calculate logistics cost for a vendor-material combination."""
    try:
        unit_price = row['AvgUnitPriceUSD_raw']
        country = row.get(country_col, '')
        material = row.get(material_col, '')
        
        # Try to get the specific combination factor
        combination_key = f"{country}_{material}"
        if combination_key in logistics_data.get('combinations', {}):
            factor = logistics_data['combinations'][combination_key]
        else:
            # Fallback to individual factors
            country_factor = logistics_data.get('country_factors', {}).get(
                country, logistics_data.get('defaults', {}).get('country_factor', 0.7)
            )
            material_factor = logistics_data.get('material_factors', {}).get(
                material, logistics_data.get('defaults', {}).get('material_factor', 0.5)
            )
            # Add some randomness
            import random
            random_factor = random.uniform(0.8, 1.2)
            factor = country_factor * material_factor * random_factor
        
        # Calculate logistics cost
        logistics_cost = unit_price * base_rate * factor
        return logistics_cost
        
    except Exception as e:
        print(f"Warning: Error calculating logistics cost: {e}")
        # Return default logistics cost
        return row.get('AvgUnitPriceUSD_raw', 0) * base_rate * 0.35

def load_country_tariffs(json_path):
    """Load tariff values from JSON file."""
    try:
        if os.path.exists(json_path):
            with open(json_path, 'r') as f:
                tariffs = json.load(f)
            print(f"Loaded country tariffs from {json_path}: {len(tariffs)} countries configured")
            return tariffs
        else:
            print(f"Warning: Country tariff file not found at {json_path}. Using default 0% tariffs.")
            return {}
    except Exception as e:
        print(f"Error loading country tariffs: {e}")
        return {}

def load_logistics_factors(json_path):
    """Load logistics cost factors from JSON file."""
    try:
        if os.path.exists(json_path):
            with open(json_path, 'r') as f:
                logistics_data = json.load(f)
            print(f"Loaded logistics factors from {json_path}")
            return logistics_data
        else:
            print(f"Warning: Logistics factors file not found at {json_path}. Using defaults.")
            return {
                'base_rate': 0.15,
                'country_factors': {},
                'material_factors': {},
        ...
        print(f"Error loading logistics factors: {e}")
        return {
            'base_rate': 0.15,
            'country_factors': {},
            'material_factors': {},
...
def main():
    ...
    parser.add_argument('--country-tariffs-path', required=False, help='Path to the JSON file with country-specific tariff values.')

    args = parser.parse_args()

    # Declare METRIC_WEIGHTS as global before modifying it
    ...
    # Load cost configuration and EIP parameters
    costs_config = {}
    global EIP_ANNUAL_HOLDING_COST_RATE, EIP_DailyHoldingCostRate_Param, EIP_SafetyStockMultiplierForLTVar_Param
    global EIP_RiskPremiumFactorForPriceVolatility_Param, EIP_PlanningHorizonDaysForPriceTrend_Param, PRICE_TREND_CUTOFF_DAYS
    global EIP_BaseLogisticsCostRate_Param
    ...
    try:
        ...
        if 'cost_components' in config_data:
            ...
            costs_config = config_data.get('cost_components', {})
            eip_params = config_data.get('economic_impact_parameters', {})
            
            # Update EIP parameters if provided
            if 'EIP_ANNUAL_HOLDING_COST_RATE' in eip_params:
                EIP_ANNUAL_HOLDING_COST_RATE = eip_params['EIP_ANNUAL_HOLDING_COST_RATE']
                EIP_DailyHoldingCostRate_Param = EIP_ANNUAL_HOLDING_COST_RATE / 365.0
            if 'EIP_SafetyStockMultiplierForLTVar_Param' in eip_params:
                EIP_SafetyStockMultiplierForLTVar_Param = eip_params['EIP_SafetyStockMultiplierForLTVar_Param']
            if 'EIP_RiskPremiumFactorForPriceVolatility_Param' in eip_params:
                EIP_RiskPremiumFactorForPriceVolatility_Param = eip_params['EIP_RiskPremiumFactorForPriceVolatility_Param']
            if 'EIP_PlanningHorizonDaysForPriceTrend_Param' in eip_params:
                EIP_PlanningHorizonDaysForPriceTrend_Param = eip_params['EIP_PlanningHorizonDaysForPriceTrend_Param']
            if 'PRICE_TREND_CUTOFF_DAYS' in eip_params:
                PRICE_TREND_CUTOFF_DAYS = eip_params['PRICE_TREND_CUTOFF_DAYS']
            if 'EIP_BaseLogisticsCostRate_Param' in eip_params:
                EIP_BaseLogisticsCostRate_Param = eip_params['EIP_BaseLogisticsCostRate_Param']
        else:
            # Old format - just cost components
            costs_config = config_data
            
        print(f"Loaded cost configuration from: {args.costs_config_path}")
        print(f"Economic Impact Parameters:")
        print(f"  Annual Holding Cost Rate: {EIP_ANNUAL_HOLDING_COST_RATE*100:.2f}%")
        print(f"  Daily Holding Cost Rate: {EIP_DailyHoldingCostRate_Param*100:.4f}%")
        print(f"  Safety Stock Multiplier: {EIP_SafetyStockMultiplierForLTVar_Param}")
        print(f"  Risk Premium Factor: {EIP_RiskPremiumFactorForPriceVolatility_Param*100:.0f}%")
        print(f"  Planning Horizon Days: {EIP_PlanningHorizonDaysForPriceTrend_Param}")
        print(f"  Price Trend Cutoff Days: {PRICE_TREND_CUTOFF_DAYS}")
        print(f"  Base Logistics Cost Rate: {EIP_BaseLogisticsCostRate_Param*100:.2f}%")
    except FileNotFoundError:
        print(f"Warning: Costs configuration file not found at {args.costs_config_path}. Using default EIP values and all cost components enabled.", file=sys.stderr)
    except json.JSONDecodeError:
        print(f"Warning: Could not decode JSON from {args.costs_config_path}. Using default EIP values and all cost components enabled.", file=sys.stderr)
    # Defaulting to True for all known components if file is missing/corrupt is handled in the calculation loop

    # Load country tariffs
    country_tariffs_path = args.country_tariffs_path or os.path.join(args.tables_dir, 'tariff_values.json')
    country_tariffs = load_country_tariffs(country_tariffs_path)
    
    # Load logistics factors
    logistics_factors_path = os.path.join(args.tables_dir, 'logistics_factors.json')
    logistics_data = load_logistics_factors(logistics_factors_path)
    ...
    col_supplier_country = column_map.get('Supplier_Country') or 'LAND1'  # Added for country-based tariffs
    ...
    required_data = {
        ...
        'Supplier': {'file': table_map.get('SAP_VLY_IL_SUPPLIER.csv', 'SAP_VLY_IL_SUPPLIER.csv'), 'cols': [col_supplier_id, col_supplier_name, col_supplier_country]},  # Added country column
    ...
    for concept, info in required_data.items():
        ...
        # Construct full path from tables_dir
    ...
    dfs_list = [df_items, df_header, df_history, df_schedule, df_supplier, df_desc]
    for df in dfs_list:
    ...
    # Track initial PO Items count
    initial_items_count = len(df_items)
    print(f"Starting with {initial_items_count:,} PO Items rows")
    
    # Filter 1: Remove rows with missing essential data
    df_items.dropna(subset=required_cols_items_mapped, inplace=True)
    after_dropna_count = len(df_items)
    print(f"After removing rows with missing essential data: {after_dropna_count:,} rows (removed {initial_items_count - after_dropna_count:,})")
    
    # Filter 2: Remove rows with invalid price units and quantities
    if col_price_unit in df_items.columns: 
        before_price_filter = len(df_items)
        df_items = df_items[df_items[col_price_unit] > 0]
        after_price_filter = len(df_items)
        print(f"After filtering price unit > 0: {after_price_filter:,} rows (removed {before_price_filter - after_price_filter:,})")
        
    if col_item_qty in df_items.columns: 
        before_qty_filter = len(df_items)
        df_items = df_items[df_items[col_item_qty] > 0]
        after_qty_filter = len(df_items)
        print(f"After filtering item quantity > 0: {after_qty_filter:,} rows (removed {before_qty_filter - after_qty_filter:,})")
    # Clean supporting tables
    header_before = len(df_header)
    df_header.dropna(subset=[col_po_num, col_supplier_id, col_po_date, col_currency], inplace=True)
    print(f"PO Header: {len(df_header):,} rows (removed {header_before - len(df_header):,} with missing data)")
    
    history_before = len(df_history)
    df_history.dropna(subset=[col_po_num, col_po_item, col_hist_post_date, col_hist_qty, col_hist_event_type], inplace=True)
    print(f"PO History: {len(df_history):,} rows (removed {history_before - len(df_history):,} with missing data)")
    
    schedule_before = len(df_schedule)
    df_schedule.dropna(subset=[col_po_num, col_po_item, col_sched_deliv_date], inplace=True)
    print(f"PO Schedule: {len(df_schedule):,} rows (removed {schedule_before - len(df_schedule):,} with missing data)")
    ...
    before_desc_merge = len(df_items)
    df_items_with_desc = pd.merge(df_items, df_desc, on=desc_key_mapped, how='left')
    print(f"    After description merge: {len(df_items_with_desc):,} rows (started with {before_desc_merge:,})")
    ...
    # MEINS Consistency Check - REMOVED for consistency with optimization script
    # The MEINS filtering is now handled only in the optimization script to ensure
    # consistent data filtering between evaluation and optimization phases
    if mode in ['matnr', 'maktx']:
        print(f"MEINS consistency check for mode '{mode}' - delegated to optimization script for data consistency")
        # Verify columns exist but don't filter
        if col_order_unit not in df_items_with_desc.columns or grouping_key_mapped not in df_items_with_desc.columns:
    ...
    before_header_merge = len(df_items_with_desc)
    df_merged = pd.merge(df_items_with_desc, df_header, on=col_po_num, how='inner')
    print(f"    After header merge (inner): {len(df_merged):,} rows (started with {before_header_merge:,}, removed {before_header_merge - len(df_merged):,})")
    ...
    if not df_gr.empty:
        ...
        before_gr_merge = len(df_merged)
        df_merged = pd.merge(df_merged, df_gr_agg, on=[col_po_num, col_po_item], how='left')
        print(f"    After goods receipt merge: {len(df_merged):,} rows (no change expected with left join)")
    else:
    ...
    before_schedule_merge = len(df_merged)
    df_merged = pd.merge(df_merged, df_schedule_agg, on=[col_po_num, col_po_item], how='left')
    print(f"    After schedule merge: {len(df_merged):,} rows (no change expected with left join)")

    print("  Merging with Supplier Info...")
    before_supplier_merge = len(df_merged)
    df_merged = pd.merge(df_merged, df_supplier, on=col_supplier_id, how='left')
    print(f"    After supplier merge: {len(df_merged):,} rows (no change expected with left join)")

    # Apply country-based tariff values (instead of merging with tariff results)
    print("  Applying country-based tariff values...")
    df_merged['TariffImpact_raw_percent'] = 0.0  # Default to 0%

    # Apply tariffs based on supplier country
    if col_supplier_country in df_merged.columns and country_tariffs:
        # Create a mapping function to get tariff percentage by country code
        def get_country_tariff(country_code):
            if pd.isna(country_code) or country_code == '':
                return 0.0
            return float(country_tariffs.get(country_code, 0.0))
        
        # Apply the mapping function to create TariffImpact_raw_percent column
        df_merged['TariffImpact_raw_percent'] = df_merged[col_supplier_country].apply(get_country_tariff)
        
        # Log the applied tariffs
        applied_tariffs = df_merged.groupby(col_supplier_country)['TariffImpact_raw_percent'].first().to_dict()
        print(f"Applied tariffs by country: {applied_tariffs}")
    else:
        print(f"Warning: Cannot apply country tariffs. Either '{col_supplier_country}' column is missing or no tariffs are configured.")
    ...
    # Filter out same-day deliveries (LeadTimeDays == 0) as potential data quality issues
    # COMMENTED OUT: LeadTimeDays == 0 filtering to restore original behavior
    # print("Filtering out same-day deliveries (LeadTimeDays == 0)...")
    # before_leadtime_filter = len(df_merged)
    # df_merged = df_merged[df_merged['LeadTimeDays'] != 0]
    # after_leadtime_filter = len(df_merged)
    # print(f"After filtering LeadTimeDays == 0: {after_leadtime_filter:,} rows (removed {before_leadtime_filter - after_leadtime_filter:,})")
    ...
    before_final_filter = len(df_merged)
    df_merged.dropna(subset=[col_supplier_id, grouping_key_mapped, 'UnitPriceUSD'], inplace=True)
    print(f"After removing rows where key metrics couldn't be calculated: {len(df_merged):,} rows (removed {before_final_filter - len(df_merged):,})")
    ...
    # Add country column to grouping and output
    if col_supplier_country in df_merged.columns:
        if col_supplier_country not in groupby_cols_mapped:
            groupby_cols_mapped.append(col_supplier_country)
    ...
    agg_definitions = {
        ...
        'TariffImpact_raw_percent': pd.NamedAgg(column='TariffImpact_raw_percent', aggfunc='mean'),
        # Ensure col_po_num is the correct column name from df_merged
        'POLineItemCount': pd.NamedAgg(column=col_po_num, aggfunc='nunique')
    }


    before_aggregation = len(df_merged)
    df_rank = df_merged.groupby(groupby_cols_mapped, as_index=False).agg(**agg_definitions)
    print(f"After aggregation by vendor/material: {len(df_rank):,} unique combinations (reduced from {before_aggregation:,} line items)")
    
    # --- Filter to only include materials with multiple suppliers ---
    print("Filtering to only include materials with multiple suppliers...")
    suppliers_per_material = df_rank.groupby(grouping_key_mapped)[col_supplier_id].nunique()
    multi_supplier_materials = suppliers_per_material[suppliers_per_material > 1].index
    
    before_multi_filter = len(df_rank)
    df_rank = df_rank[df_rank[grouping_key_mapped].isin(multi_supplier_materials)]
    
    print(f"Materials with multiple suppliers: {len(multi_supplier_materials):,}")
    print(f"After filtering for multi-supplier materials: {len(df_rank):,} combinations (removed {before_multi_filter - len(df_rank):,})")
    print(f"Total data reduction: {initial_items_count:,} → {len(df_rank):,} ({(1 - len(df_rank)/initial_items_count)*100:.1f}% reduction)")
    ...
    # --- Handle Outliers using Percentile-based capping (Before Filling other NaNs) ---
    # Operates on the _raw metrics
    print("Handling outliers using percentile-based capping...")
    ...
    for metric in raw_metrics_for_cleaning:
        if metric in df_rank.columns:
            # Use different percentiles for different metrics
            if metric == 'AvgUnitPriceUSD_raw':
                # For prices, use 99th percentile to preserve legitimate high-value items
                lower_percentile = 0.01  # 1st percentile
                upper_percentile = 0.99  # 99th percentile
            else:
                # For other metrics, use 95th percentile
                lower_percentile = 0.05  # 5th percentile
                upper_percentile = 0.95  # 95th percentile
            
            lower_bound = df_rank[metric].quantile(lower_percentile)
            upper_bound = df_rank[metric].quantile(upper_percentile)
            
            if pd.isna(lower_bound) or pd.isna(upper_bound):
                print(f"  Skipping outlier capping for '{metric}' (Cannot calculate percentiles).")
                continue
            ...
            if num_outliers_lower > 0 or num_outliers_upper > 0:
                print(f"  Capping outliers for '{metric}' using P{int(lower_percentile*100)}/P{int(upper_percentile*100)}: Lower (<{lower_bound:.4f}): {num_outliers_lower}, Upper (>{upper_bound:.4f}): {num_outliers_upper}")
                df_rank[metric] = df_rank[metric].clip(lower=lower_bound, upper=upper_bound)
            else:
                print(f"  No outliers found for '{metric}' at P{int(lower_percentile*100)}/P{int(upper_percentile*100)} thresholds")
        else:
    ...
    # Calculate Logistics Cost
    print("Calculating logistics cost component...")
    df_rank['cost_Logistics'] = df_rank.apply(
        lambda row: calculate_logistics_cost(
            row,
            col_supplier_country,
            grouping_key_mapped,
            logistics_data,
            EIP_BaseLogisticsCostRate_Param
        ),
        axis=1
    )
    ...
    #     'cost_BasePrice', 'cost_Tariff', 'cost_Holding_LeadTime',
    #     'cost_Holding_LTVariability', 'cost_Holding_Lateness',
    #     'cost_Risk_PriceVolatility', 'cost_Impact_PriceTrend', 'cost_Logistics'
    # ]
    ...
    if col_supplier_country in df_rank.columns:
        output_cols_base.append(col_supplier_country)
    ...
    try:
        # Ensure consistent data types before saving to prevent downstream dtype warnings
        df_output = df_rank[output_cols_present].copy()
        
        # Ensure LIFNR (supplier ID) is consistently saved as string
        if col_supplier_id in df_output.columns:
            df_output[col_supplier_id] = df_output[col_supplier_id].astype(str)
        
        # Ensure material-related ID columns are consistently saved as strings
        if col_mat_num in df_output.columns:
            df_output[col_mat_num] = df_output[col_mat_num].astype(str)
        if grouping_key_mapped in df_output.columns and grouping_key_mapped != col_mat_num:
            df_output[grouping_key_mapped] = df_output[grouping_key_mapped].astype(str)
        
        df_output.to_csv(output_filename, index=False, float_format='%.4f')
