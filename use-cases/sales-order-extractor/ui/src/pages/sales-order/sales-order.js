...
class SalesOrderPage {
  ...
  attachEventListeners() {
    ...
    // Attach listeners immediately, don't wait for component upgrades
    this.setupEventListeners();
  }

  // Helper to ensure components are ready if needed for specific operations
  async ensureComponent(tagName) {
  ...
  async handleSave() {
    ...
    try {
      ...
      // Mocking API call to avoid proxy errors
      /*
  ...
  showMessage(message, type) {
    ...
    if (type === "Success") {
      ...
      if (dialog && messageText) {
        messageText.textContent = message;

        // Ensure the dialog is closed before opening it again to trigger the opening animation/logic
        if (dialog.open) {
          dialog.open = false;
          setTimeout(() => {
            dialog.open = true;
