...
def run_chatbot_interaction(all_data):
    """Orchestrates the interactive chatbot conversation flow in English."""
    print("\n--- Welcome to the Apex Automotive Services After-Sales Chatbot ---")
    print("I can help you check your vehicles, service history, promotions, or schedule an appointment.")
    ...
    while True:
        ...
        parsed_result = parse_intent_llm(user_input, chat_history)
        ...
        # Using renamed intents and entity keys from parse_intent_llm
        if intent == "identify_client":
            ...
            if identifier_type and identifier_value:
                ...
                if client_id:
                    conversation_state["client_id"] = client_id
                    conversation_state["selected_vin"] = None # Reset selected VIN on new identification
        ...
        # --- Handle Actions Requiring Client ID ---
        elif intent in ["list_units", "select_unit"] and not conversation_state["client_id"]:
            ...
            if units:
             ...
             if not vin_to_select and bot_response == "Sorry, I didn't understand that. Could you rephrase?":
                 ...
                 client_units = get_client_units(all_data, conversation_state["client_id"])
                 valid_vins = [u['Vin'].upper() for u in client_units]
                 if vin_to_select.upper() in valid_vins:
                     conversation_state["selected_vin"] = vin_to_select.upper()
                     unit_details = get_unit_details(all_data, conversation_state["selected_vin"])
                     details_str = f"{unit_details.get('Marca')} {unit_details.get('Modelo')} {unit_details.get('Año')}" if unit_details else conversation_state["selected_vin"]
                     
                     prompt = f"The user selected the vehicle VIN: {conversation_state['selected_vin']} ({details_str}). Confirm the selection in English and ask what they need to know about it (details, history, promotions, next service, schedule appointment)."
                     bot_response = generate_response_llm(prompt)
                 else:
        ...
        # Using renamed intents
            ...
            elif conversation_state.get("selected_vin"): target_vin = conversation_state["selected_vin"]
            ...
            if not target_vin and bot_response == "Sorry, I didn't understand that. Could you rephrase?": # Only ask if no other error occurred
                 ...
                 vin_is_valid = True
                 if conversation_state["client_id"]:
                     client_units = get_client_units(all_data, conversation_state["client_id"])
                     valid_vins = [u['Vin'].upper() for u in client_units]
                 ...
                 if vin_is_valid:
                    if target_vin.upper() != conversation_state.get("selected_vin"):
                         conversation_state["selected_vin"] = target_vin.upper()
                         print(f"DEBUG: Updated selected VIN to {conversation_state['selected_vin']}")
...
- If specific kit information was not found, indicate that details couldn't be obtained but still recommend the service based on mileage.
"""
                            bot_response = generate_response_llm(prompt)
        ...
        if bot_response == "Sorry, I didn't understand that. Could you rephrase?":
             ...
             prompt = f"Context:\n{history_context_str}\n\nThe intent of the user's last message could not be determined or there was an error. Apologize in English and ask the user to rephrase their request."
             bot_response = generate_response_llm(prompt)
        ...
        print(f"Chatbot: {bot_response_content}")
        conversation_state["last_intent"] = intent

# --- Main Execution ---
if __name__ == "__main__":
    # Load data using the relative path expected by the module structure
    # The TABLES_PATH in config.py should point correctly relative to the project root
    all_data = load_data()

    if all_data and llm: # Check if data loaded AND llm initialized
        run_chatbot_interaction(all_data)
    elif not all_data:
        print("\nFailed to load data. Chatbot cannot start.") 
    else: # llm failed to initialize
        print("\nLLM initialization failed. Chatbot cannot start.")