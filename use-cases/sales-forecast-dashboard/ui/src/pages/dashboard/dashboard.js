...
function initMap() {
  // Create map and derive the initial viewport from the loaded geometry.
  state.map = L.map("map");
  ...
  resetMapViewToDataBounds();
  handleZoomChange();
}

function getValidMapPoints(records) {
  return records
    .filter(record => Number.isFinite(record.lat) && Number.isFinite(record.lng))
    .map(record => [record.lat, record.lng]);
}

function getInitialMapPoints() {
  const storePoints = getValidMapPoints(state.storesData);
  if (storePoints.length > 0) {
    return storePoints;
  }

  return getValidMapPoints(state.dmaData);
}

function resetMapViewToDataBounds() {
  const points = getInitialMapPoints();

  if (points.length === 0) {
    state.map.setView(DEFAULT_MAP_CENTER, INITIAL_ZOOM);
    return;
  }

  if (points.length === 1) {
    state.map.setView(points[0], ZOOM_THRESHOLD + 1);
    return;
  }
