...
export default function initChatbotPage() {
  ...
  const disclaimerDialog = document.getElementById("ai-disclaimer-dialog");
  const disclaimerOptOut = document.getElementById("ai-disclaimer-optout");
  const disclaimerOk = document.getElementById("ai-disclaimer-ok");
  const disclaimerCancel = document.getElementById("ai-disclaimer-cancel");
  const disclaimerKey = "ai_disclaimer_acknowledged";
  ...
  // Show disclaimer dialog unless user opted out previously
  function maybeShowDisclaimer() {
    if (!disclaimerDialog) return;
    const acknowledged = localStorage.getItem(disclaimerKey) === "true";
    if (!acknowledged) {
      disclaimerDialog.open = true;
    }
  }

  function closeDisclaimer(acknowledge) {
    if (!disclaimerDialog) return;
    if (acknowledge && disclaimerOptOut && disclaimerOptOut.checked) {
      localStorage.setItem(disclaimerKey, "true");
    }
    disclaimerDialog.open = false;
  }

  if (disclaimerOk) {
    disclaimerOk.addEventListener("click", () => closeDisclaimer(true));
  }

  if (disclaimerCancel) {
    disclaimerCancel.addEventListener("click", () => closeDisclaimer(false));
  }
        ...
        if (attachment.file_type === "image") {
          ...
          const iconName =
            attachment.file_type === "pdf"
              ? "pdf-attachment"
              : attachment.file_type === "docx"
                ? "doc-attachment"
                : "excel-attachment";
          downloadIcon.setAttribute("name", iconName);
