...
def _append_report_footer(pdf, text: str) -> None:
    """
    Append a short footer note at the end of the PDF.

    Args:
        pdf: FPDF instance
        text: Footer text to render
    """
    min_space = 12
    if pdf.get_y() > (pdf.h - pdf.b_margin - min_space):
        pdf.add_page()
    else:
        pdf.ln(4)
...
def markdown_to_pdf(
    ...
    _append_report_footer(pdf, "This report is AI Generated.")
...
def _add_runs_with_inline_bold(paragraph, text: str) -> None:
    """
    Add runs to a paragraph, handling **bold** segments.
    """
    parts = text.split("**")
...
def _apply_heading_style(paragraph, level: int) -> None:
    """
    Apply heading styling to a paragraph to mimic PDF look.
    """
...
def _render_markdown_table_docx(doc, table_lines: List[str]) -> None:
    """
    Render a markdown table in a DOCX document.
    """
    if len(table_lines) < 2:
        return
...
def markdown_to_docx(
    ...
    """
    Convert LLM-generated markdown to DOCX with embedded plots.
    """
    ...
    # Set default font to Open Sans if available
    ...
    doc.add_paragraph("This report is AI Generated.")
...
def generate_report(
    ...
    # 9. Convert markdown to PDF and DOCX with embedded plots
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_name = f"forecast_report_{timestamp}"
        ...
        try:
            ...
            session.add_export_file(str(docx_path.absolute()))
        except Exception:
            pass  # Don't fail if tracking fails
    except ImportError:
        return {
            "error": "fpdf2 or python-docx library not installed. Run: pip install fpdf2 python-docx",
            "hint": "Install the PDF and Word generation libraries.",
        }
    except Exception as e:
        return {
