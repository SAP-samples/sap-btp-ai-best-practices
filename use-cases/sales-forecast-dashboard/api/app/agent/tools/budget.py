...
def _prepare_monthly_panel() -> pd.DataFrame:
    ...
    # Filter Bob's Discount Furniture if Company column exists
    if company_col:
        awareness_df = awareness_df[awareness_df[company_col] == "Bob's Discount Furniture"].copy()

    logger.info(f"After company filter: {len(awareness_df)} rows")

    if awareness_df.empty:
        raise ValueError(
            "No awareness data found for Bob's Discount Furniture. "
