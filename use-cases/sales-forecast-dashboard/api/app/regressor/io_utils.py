...
def load_demographics() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    static_col_map = {
        ...
        'Bobs Store within 20 Miles': 'bobs_stores_nearby'
    }
...
def load_competitor_density() -> pd.DataFrame:
    """Compute Bob's store density metrics (distances to other Bob's stores).

    Uses geo.py::compute_store_neighbors() to calculate pairwise distances
    between all Bob's stores.

    Returns:
        DataFrame with columns:
        - profit_center_nbr (int): Store ID
        - min_distance_to_bobs (float): Miles to nearest Bob's store
        - bobs_count_10mi (int): Count of Bob's stores within 10 miles
        - bobs_count_20mi (int): Count of Bob's stores within 20 miles
        - bobs_count_30mi (int): Count of Bob's stores within 30 miles
    """
    ...
    result = result.rename(columns={
        'min_distance_miles': 'min_distance_to_bobs',
        'count_10mi': 'bobs_count_10mi',
        'count_20mi': 'bobs_count_20mi',
        'count_30mi': 'bobs_count_30mi',
    })

    # Convert counts to int, filling NaN with 0
    for col in ['bobs_count_10mi', 'bobs_count_20mi', 'bobs_count_30mi']:
