...
def _prepare_sales_with_dma(use_written_sales_for_web: bool = False) -> pd.DataFrame:
    """
    ...
    - WEB Sales: Ecomm Traffic.csv (merch_amt is authoritative) [default]
                 OR Written Sales Data.csv WEB rows (if use_written_sales_for_web=True)
    - WEB Orders: Written Sales Data.csv WEB rows (for AOV calculation only)

    Args:
        use_written_sales_for_web: If True, use Written Sales WEB rows for total_sales
                                   instead of Ecomm Traffic merch_amt. Default False.
    ...
    # === WEB DATA ===
    if use_written_sales_for_web:
        # Use Written Sales WEB rows for both sales and orders
        web_sales = written_sales[written_sales["channel"] == "WEB"].copy()
        web_sales = web_sales.merge(stores, on="profit_center_nbr", how="left")
        web_sales = web_sales.rename(columns={"market_city": "dma"})
        web_sales["origin_week_date"] = pd.to_datetime(
            web_sales["fiscal_start_date_week"], errors="coerce"
        )
    else:
        # Default: Use Ecomm Traffic for sales, Written Sales for orders
        ...
        web_written["fiscal_start_date_week"] = pd.to_datetime(
            web_written["fiscal_start_date_week"], errors="coerce"
        )
...
def build_canonical_training_table(
    ...
    model_variant: str = 'B',
    include_crm: bool = False,
    use_written_sales_for_web: bool = False,
) -> pd.DataFrame:
    """
    ...
    sales = _prepare_sales_with_dma(use_written_sales_for_web=use_written_sales_for_web)
