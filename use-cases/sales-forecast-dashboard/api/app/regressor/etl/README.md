...
This module provides the **single source of truth** for building the canonical training table used for Bob's sales forecasting models. It implements both Epic 3 (ETL pipeline) and Epic 4 (Feature Engineering) requirements from `PROJECT_PLAN.md` and follows specifications in `ENGINEERING_INSTRUCTIONS.md`.
...
**Rationale**:
- WEB has no physical traffic (Features - Bob.md line 89)
...
- **`Features - Bob.md`**: Target variable specifications (lines 77-111)
- **`Model - Bob.md`**: Step 2 - Canonical Training Table (lines 12-31)
