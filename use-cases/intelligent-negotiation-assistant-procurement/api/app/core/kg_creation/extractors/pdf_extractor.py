...
This module handles PDF text extraction with enhanced metadata tracking,
preserving structural context through page separators for downstream processing.
...
def extract_pdf_content(file_path: str, method: str = "pymupdf", include_metadata: bool = True) -> Dict[str, Any]:
    ...
    try:
        if method == "pymupdf":
            result = _extract_with_pymupdf(file_path, result, include_metadata)
        elif method == "pdfplumber":
            result = _extract_with_pdfplumber(file_path, result, include_metadata)
        else:
...
def _extract_with_pymupdf(file_path: Path, result: Dict[str, Any], include_metadata: bool = True) -> Dict[str, Any]:
    """Extract text using PyMuPDF with enhanced formatting and metadata preservation."""
    ...
    with fitz.open(str(file_path)) as pdf_doc:
        ...
        for page_num in range(len(pdf_doc)):
            ...
            # Store per-page content with enhanced metadata
            page_data = {
                "page_number": page_num + 1,
                "text": final_page_text,
                "character_count": len(final_page_text),
                "word_count": len(re.findall(r'\b\w+\b', final_page_text))
            }
...
def _extract_with_pdfplumber(file_path: Path, result: Dict[str, Any], include_metadata: bool = True) -> Dict[str, Any]:
    """Extract text using pdfplumber with table detection and metadata tracking."""
...
def extract_pdf_for_kg_pipeline(file_path: str) -> Dict[str, Any]:
    ...
    return extract_pdf_content(
        file_path=file_path,
        method="pymupdf",  # Default to PyMuPDF for better formatting
