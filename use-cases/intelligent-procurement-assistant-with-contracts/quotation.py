...
#############################################################################
# SAP AI HUB - PRODUCTION STRICT
#############################################################################


def test_sap_ai_hub_connection():
    """Test SAP AI Hub connectio"""
    try:
        print("DEBUG: Testing SAP AI Hub connection...")
        ...
        if "invalid_client" in str(e) or "Bad credentials" in str(e):
            st.error("SAP AI Hub Authentication Failed")
            st.error("Check  credentials in code:")
            st.code(
                """
...
def ask_llm_production(pdf_text: str, vendors_df: pd.DataFrame) -> str:
    """Ask LLM with credentials - UPDATED: No commodity code detection"""
    ...
    try:
        ...
        print("DEBUG: Creating SAP AI Hub clients with credentials...")
        proxy_client = get_proxy_client("gen-ai-hub")
        llm = ChatOpenAI(proxy_model_name="gpt-4.1", proxy_client=proxy_client)
...
- Authentication token expired
            ...
            st.error("Unauthorized (401) - Credentials failed")
            st.code("Update AICORE_CLIENT_ID and AICORE_CLIENT_SECRET")
        elif "429" in str(e):
...
def analyze_quotation_with_llm(
    ...
    print("=" * 50)
    print("STARTING QUOTATION ANALYSIS - CREDENTIALS")
    print("=" * 50)
...
def simulate_pr_creation(
    ...
    """Simulate PO creation - FIXED BUG"""
...
def show_pr_creation_success():
    ...
    with col2:
        st.markdown("### Purchase Order Created")

        info_text = f"""
        **PO Number:** {pr_data.get('pr_number', 'N/A')}
...
def show_quotation_analysis_results(quotation_data: Dict, vendors_df: pd.DataFrame):
    ...
    if not vendor_found:
        ...
        **To proceed with this quotation:**
        ...
        st.markdown("---")
        st.warning("Purchase Order creation is disabled until vendor is approved")
    ...
    delivery_address = st.text_area(
        ...
        help="Enter the delivery address for this purchase order",
    )
    ...
    with col3:
        if st.button("Confirm PO", use_container_width=True, type="primary"):
...
def handle_quotation_upload(use_sap_ai_hub: bool = True):
    ...
    print("DEBUG: handle_quotation_upload called - CREDENTIALS")
