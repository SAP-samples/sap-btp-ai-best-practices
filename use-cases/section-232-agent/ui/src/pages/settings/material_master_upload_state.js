...
const MATERIAL_MASTER_WORKBOOK_EXTENSIONS = [".xlsb", ".xlsx"];
const MATERIAL_MASTER_WORKBOOK_EXTENSION_COPY = ".xlsb or .xlsx";

/**
 * Determine whether a browser File-like object is a supported Material Master Excel workbook.
 *
 ...
 */
export function isMaterialMasterWorkbookFile(file) {
  const filename = String(file?.name || "").trim().toLowerCase();
  return MATERIAL_MASTER_WORKBOOK_EXTENSIONS.some((extension) => filename.endsWith(extension));
}

/**
 * Build Settings-page copy for the currently selected Material Master file.
 *
 ...
 */
export function buildMaterialMasterSelectionText(file) {
  if (!file) {
    return `No Material Master ${MATERIAL_MASTER_WORKBOOK_EXTENSION_COPY} file selected.`;
  }
  return `Selected file: ${file.name || "Material Master workbook"} (${formatFileSize(file.size)}).`;
}

/**
 * Build the post-refresh success message shown on the Settings page.
 *
 * @param {object} response Material Master refresh API response payload.
 * @returns {string} Human-readable refresh and cleanup result.
 */
export function buildMaterialMasterRefreshSuccessMessage(response = {}) {
  const filename = response.uploaded_filename || "Material Master file";
  ...
  return (
    `Uploaded ${filename} and refreshed ${rowCount} Material Master ${pluralize(rowCount, "row")} in HANA. ` +
