---
title: "Pro-Code AI Agents"
description: "Best practices for building code-based AI agents with a pro-code approach."
hide_title: true
---
    ...
    <div className="col col--4">
      <ul className="resource-links">
        <li>
          <a
            target="_blank"
            href={useBaseUrl("agentic-ai/pro-code-ai-agents/videos/BTP%20AI%20Best%20Practices%20-%20Pro-Code%20Agents%20(intro)_external.mp4")}
          >
            <Icon name="video" />
            <span>Teaser</span>
          </a>
        ...
        <LoginWall renderOnlyWhenLoggedIn={true}>
          <li>
            <a
              target="_blank"
              href={useBaseUrl("agentic-ai/pro-code-ai-agents/videos/BTP%20AI%20Best%20Practices%20-%20Pro-Code%20Agents-compressed.mp4")}
            >
              <Icon name="video" />
              <span>Webinar</span>
            </a>
          ...
          <li>
            <a
              target="_blank"
              href={useBaseUrl("agentic-ai/pro-code-ai-agents/pdfs/BTP%20AI%20Best%20Practices%20-%20Pro-Code%20Agents.pdf")}
            >
...
<LoginWall renderOnlyWhenLoggedIn={true}>
  <TrackableLink
    href="https://github.com/SAP-samples/sap-btp-ai-best-practices/tree/main/best-practices/agentic-ai/code-based-agents"
    className="button button--primary button--lg download-source-btn"
    target="_blank"
    trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS"
  >
...
This best practice page focuses on building production-ready AI agents using code-based approaches within the SAP ecosystem. For security and governance of AI agents, see the dedicated <Link to={useBaseUrl("/docs/technical-view/agentic-ai/agentic-ai-security-governance")}>**Agentic AI Security and Governance**</Link> best practice page. For connecting agents to enterprise tools and data sources via the Model Context Protocol, see the <Link to={useBaseUrl("/docs/technical-view/agentic-ai/mcp")}>**Model Context Protocol (MCP)**</Link> best practice page.
...
* Integrate agents with SAP services, external APIs, and enterprise data sources via the SAP Cloud SDK for AI
...
* **Tools:** Functions or callable components that extend the agent's capabilities beyond text generation, e.g., calling APIs, querying databases, or performing calculations. Tools can be implemented in Python, JavaScript/TypeScript, or other supported runtimes, and are typically described with a name, description, and input schema so the LLM can select and invoke them.
...
* **Monkey Patching**: A technique that dynamically modifies or extends the behavior of a class, module, or function at runtime without changing its original source code.
...
* SAP Business Technology Platform (SAP BTP) subaccount
* SAP AI Core

For a deeper understanding of LLM model access, configuration and endpoint settings, refer to the <Link to={useBaseUrl("/docs/technical-view/generative-ai/plain/access-to-generative-ai-models")}>Access to Generative AI Models</Link> best practice page.

### High-level Reference Architecture

![High-level Reference Architecture](@site/static/agentic-ai/pro-code-ai-agents/images/high-level-reference-architecture.webp)
...
### Design Agent Context for Token Caching

**Why this matters:** ReAct agents resend their instructions, tool schemas, reference material and growing history on every model call. Prompt caching can reuse an unchanged prefix, reducing input cost.

* **Put stable content first.** Place system policies, tool definitions, examples, and reusable references before changing user and tool messages.
* **Keep the prefix deterministic.** Append new messages; avoid changing timestamps, identifiers, tool order, or earlier turns before the reusable boundary.
* **Measure at the model call.** Compare consecutive calls using total input, cache reads, cache writes, uncached input, and output tokens when those fields are reported.
* **Verify the economics.** A cache write saves nothing unless later calls reuse it. Prices, eligibility, controls and retention vary by model and API; a cache-read percentage is not the same as a cost-reduction percentage.
* **Keep concepts separate.** Caching reuses repeated input processing; it does not replace context trimming, conversation memory, or checkpoint persistence.

### Manage Checkpoints and Session State Deliberately

**Why this matters:** A LangGraph checkpointer saves one workflow's state so it can resume after a failure, pause for approval or be inspected later. It is session state, not cross-session memory for user preferences.

* **Start stateless.** Add checkpointing only when continuity, recovery, human approval, or later investigation is a real requirement.
* **Choose storage by lifecycle.** Use process-local state only for development or disposable sessions; use durable storage when state must survive restarts or scale-out.
* **Scope every session.** Use a stable `thread_id` as a lookup key, but enforce user and tenant authorization on the server; the identifier is not an access grant.
* **Separate state from model context.** Keep the full checkpoint history needed for recovery, but send the model only a summary and recent relevant events as described in the [LangGraph memory guidance](https://docs.langchain.com/oss/python/langgraph/add-memory).
* **Store long-term memory separately.** Cross-session preferences and facts belong in a deliberately governed store, not in every workflow checkpoint.
* **Govern persisted data.** Minimize and redact stored content, exclude secrets, isolate tenants, and define encryption, retention, backup, and deletion controls. See the [LangGraph persistence guide](https://docs.langchain.com/oss/python/langgraph/persistence) and [SAP BTP data protection guidance](https://help.sap.com/docs/btp/sap-business-technology-platform/data-protection-and-privacy).
...
Agentic AI introduces unique security risks including prompt injection, memory poisoning and privilege escalation. These topics are covered in depth in the dedicated <Link to={useBaseUrl("/docs/technical-view/agentic-ai/agentic-ai-security-governance")}>Agentic AI Security and Governance</Link> page. At minimum, follow SAP's guidelines on input validation, execution quotas and least-privilege access control for all agent deployments.
...
**LangGraph for Python, Java and JavaScript/TypeScript** is the recommended stack for building code-based AI agents on SAP BTP. The SAP Cloud SDK for AI (`sap-ai-sdk-gen`) provides the LLM access layer through LangChain-compatible wrappers, avoiding vendor lock-in while routing all model calls through SAP AI Core for centralized governance. All reference examples below demonstrate this combination.

### SDKs

**SAP Cloud SDK for AI for Python** (`sap-ai-sdk-gen`) (Recommended)
...
* [Getting Started with Agents Using SAP Cloud SDK for AI](https://community.sap.com/t5/devtoberfest/getting-started-with-agents-using-sap-generative-ai-hub/ev-p/13865119) (Devtoberfest session)
...
* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/tree/main/best-practices/agentic-ai/code-based-agents/native-react" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Native ReAct Notebook**</TrackableLink>
...
* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/tree/main/best-practices/agentic-ai/code-based-agents/langgraph-react" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - LangGraph ReAct Agents**</TrackableLink>
...
![Mermaid Diagram - Basic ReAct Agent](@site/static/agentic-ai/pro-code-ai-agents/images/mermaid-diagram-basic-react.webp)
...
* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/blob/main/best-practices/agentic-ai/code-based-agents/langgraph-react/01_react_agent.ipynb" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Basic ReAct Notebook**</TrackableLink>
...
![Mermaid Diagram - Multi-Agent Routing](@site/static/agentic-ai/pro-code-ai-agents/images/mermaid-diagram-multi-agent.webp)
...
* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/blob/main/best-practices/agentic-ai/code-based-agents/langgraph-react/02_multi_step_workflow.ipynb" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Multi-Agent Routing with Conditional Edges Notebook**</TrackableLink>
...
* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/blob/main/best-practices/agentic-ai/code-based-agents/langgraph-react/03_procurement_workflow.ipynb" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Enterprise Procurement Workflow Notebook**</TrackableLink>

#### 2.4 Token Caching in a ReAct Agent

This tutorial makes the cost behavior visible inside a real LangGraph tool loop. Four ordered tool operations produce five model calls, each reusing the same stable system prompt while appending assistant messages and tool results. The assistant node captures raw usage telemetry exactly where the model is invoked, so the notebook can distinguish total prompt-side volume, cache writes, cache reads, derived uncached input, and output tokens for every call and for the complete workflow.

The notebook first demonstrates implicit caching through SAP Generative AI Hub's LangChain Responses API. It then compares an explicit breakpoint placed only on the stable system content, showing why the cached portion remains fixed while the appended transcript continues to grow unless a later supported boundary is added. The goal is not to promise a fixed saving, but to show how prompt structure creates reuse opportunities and how to verify the resulting price mix from the active route's telemetry.

##### Reference Code

* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/blob/main/best-practices/agentic-ai/code-based-agents/langgraph-react/04_token_caching.ipynb" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Token Caching Notebook**</TrackableLink>

#### 2.5 Checkpointing and Session State

This tutorial starts without an LLM so the session behavior is easy to see: it runs two turns with one `thread_id`, prints the complete saved history after each turn, and proves that another thread and a restarted in-memory saver cannot access it. It then compares that full checkpointed history with the smaller message list sent to a model after trimming, showing exactly what is omitted from the request while remaining available in the saved session. The final optional ReAct example applies the same inspection pattern to a SAP Generative AI Hub tool-calling agent.

##### Reference Code

* <TrackableLink href="https://github.com/SAP-samples/sap-btp-ai-best-practices/blob/main/best-practices/agentic-ai/code-based-agents/langgraph-react/05_checkpointing_and_session_state.ipynb" target="_blank" trackingFeature="DOWNLOAD_PRO_CODE_AI_AGENTS">**SAP BTP AI Best Practices - Code-Based Agents - Checkpointing and Session State Notebook**</TrackableLink>
