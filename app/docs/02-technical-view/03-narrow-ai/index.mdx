---
title: Narrow AI
description: "Narrow AI is a specialized artificial intelligence designed to efficiently perform a specific task or a limited range of tasks. Unlike general AI, it cannot generalize its knowledge or skills to tasks beyond its designated domain."
hide_table_of_contents: true
hide_title: true
---

import IconLinkButton from "@site/src/components/IconLinkButton";
import PageViewTracker from "@site/src/components/tracking/PageViewTracker";

<PageViewTracker />

<div className="hero-header">
  <h1>Narrow AI</h1>
</div>

<div className="section-with-background blue">
  <h2>What is Narrow AI?</h2>
  <p>
    Narrow AI is a specialized artificial intelligence designed to efficiently perform a specific task or a limited range of tasks. Unlike general AI, it cannot generalize its
    knowledge or skills to tasks beyond its designated domain.
  </p>
</div>

<br />

<div className="row">
  <div className="col col--6">
    <h3>Key Predictive Functions</h3>
    <ul className="button-list">
      <IconLinkButton href="/docs/technical-view/narrow-ai/regression" text="Regression" />
      <IconLinkButton href="/docs/technical-view/narrow-ai/time-series-forecasting" text="Time Series Forecasting" />
      <IconLinkButton href="/docs/technical-view/narrow-ai/classification" text="Classification" />
      <IconLinkButton href="/docs/technical-view/narrow-ai/clustering" text="Clustering" />
      <IconLinkButton href="/docs/technical-view/narrow-ai/anomaly-detection" text="Anomaly Detection" />
    </ul>
  </div>
  <div className="col col--6">
    <h3>More</h3>
    <ul className="button-list">
      <IconLinkButton href="/docs/technical-view/narrow-ai/sap-rpt" text="Access to SAP RPT Model" disabled={true} />
      <IconLinkButton href="/docs/technical-view/narrow-ai/data-preparation-and-model-evaluation" text="Data Preparation and Model Evaluation" />
      <IconLinkButton href="/docs/technical-view/narrow-ai/training-deployment-custom-ai-models" text="Training and Deployment of Custom AI Models" />
    </ul>
  </div>
</div>
